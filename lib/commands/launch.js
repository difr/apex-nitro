...
module.exports = async function (cb) {
	...
	const apexProxyReq = function (req, res) {
		...
		req.setHeader('Origin', req.agent.protocol + '//' + (req.connection._host || req._headers.host));
	...
	const launch = function () {
		...
		const onChange = async function() {
			...
			try {
				...
				changesTimeout = void 0;
				if (changesWaiting) {
					changesTimeout = setTimeout(onChange, 2000);
					console.log(`Pending changes (${changesWaiting}), waiting 2 seconds to start build...`);
				}
			}
		}
		...
		browsersync
			...
			.on('all', () => {
				changesWaiting += 1;
				if (changesTimeout !== 'building') {
					if (changesTimeout) {
						changesTimeout = void clearTimeout(changesTimeout);
					}
					changesTimeout = setTimeout(onChange, 3000);
					console.log(`Change detected (${changesWaiting}), waiting 3 seconds to start build...`);
				} else {
					console.log(`Change detected (${changesWaiting}), waiting until build is completed...`);
				}
			});
	...
	if (config.mode !== 'basic') {
		const singleBundle = process.env.SINGLE_BUNDLE;
		if (singleBundle && singleBundle.trim().length) {
